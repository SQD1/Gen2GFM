...
from archs.aggregation_networks_clean import GlobalWeightedFuser
...
class RegUPerNetDecoderMixin:
    def __init__(
        ...
        **kwargs
        ...
        self.conv_reg = nn.Conv2d(self.channels, self.num_classes, kernel_size=1)
        self.dropout = nn.Dropout2d(0.1)
    ...
    def _forward_feature(self, inputs):
        """Forward function for feature maps before regression output."""
        # build laterals
        laterals = [
            lateral_conv(inputs[i]) for i, lateral_conv in enumerate(self.lateral_convs)
        ]
...
class GWFuserRegUPerNet(RegUPerNetDecoderMixin, GlobalWeightedFuser):
    def __init__(
            self, 
            feature_dims, 
            projection_dim=384,
