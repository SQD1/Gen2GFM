    ...
    def validate(self, epoch, global_step):
        ...
        with torch.no_grad():
            for idx, batch in tqdm(enumerate(self.val_loader), total=len(self.val_loader), leave=False):
                images = batch['rgb'].to(self.device)   # [-1, 1]
                labels = batch['label'].to(self.device)
...
# python
class ChangeEvaluator(Evaluator):
    """Evaluator for change detection (two-input images, binary classes)."""
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # enforce or assume binary change detection
        self.num_classes = 2
        self.class_names = self.cfg.class_names if hasattr(self.cfg, "class_names") else ["no_change", "change"]
        self.class_colors = {0: (0, 0, 0), 1: (255, 255, 255)}

    def validate(self, epoch, global_step):
        self.logger.info(f"Starting change-detection validation epoch {epoch} on {self.val_type} set")
        start_time = time.time()
        self.network.eval()
        ...
        with torch.no_grad():
            for idx, batch in tqdm(enumerate(self.val_loader), total=len(self.val_loader), leave=False):
                # accept common key names for the two images
                if "rgb1" in batch and "rgb2" in batch:
                    img1 = batch["rgb1"].to(self.device)
                    img2 = batch["rgb2"].to(self.device)
                elif "rgb_a" in batch and "rgb_b" in batch:
                    img1 = batch["rgb_a"].to(self.device)
                    img2 = batch["rgb_b"].to(self.device)
                else:
                    # fallback: expect a tuple/list under 'rgb'
                    rgb = batch["rgb"]
                    img1, img2 = rgb[0].to(self.device), rgb[1].to(self.device)

                labels = batch["label"].to(self.device)

                # encode both images with VAE and extract features separately
                latents1 = self.vae.encode(img1).latent_dist.sample(generator=self.g) * 0.18215
                latents2 = self.vae.encode(img2).latent_dist.sample(generator=self.g) * 0.18215
                latents1 = latents1.to(self.device)
                latents2 = latents2.to(self.device)

                with torch.inference_mode():
                    feats1, _ = self.ldm_extractor.forward(latents1)
                    feats2, _ = self.ldm_extractor.forward(latents2)

                    # combine features per scale; support single-tensor or list-of-tensors
                    if torch.is_tensor(feats1) and torch.is_tensor(feats2):
                        combined_feats = feats2 - feats1
                    else:
                        combined_feats = [f2 - f1 for f1, f2 in zip(feats1, feats2)]

                    logits, _ = self.network(combined_feats, output_shape=(self.cfg.original_img_size, self.cfg.original_img_size))
                    preds = torch.argmax(logits, dim=1)

                # handle ignore index and accumulate confusion
                mask = (labels != self.ignore_index) if self.ignore_index is not None else torch.ones_like(labels, dtype=torch.bool)
                valid_preds = preds[mask]
                valid_labels = labels[mask]
        ...
        end_time = time.time()
        self.logger.info(f"Validation epoch {epoch} completed in {end_time - start_time:.2f} seconds")
        return metrics["F1"][1]  # return F1 for 'change' class
...
class RegressionEvaluator(Evaluator):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.use_slide_window_inference = bool(
            self.cfg.get("slide_window_inference", self.cfg.get("use_slide_window_inference", False))
        )
        self.slide_window_size = int(self.cfg.img_size)
        self.slide_stride = int(self.cfg.get("slide_window_stride", self.slide_window_size))

    @staticmethod
    def _compute_starts(size, window_size, stride):
        """Compute sliding window start offsets and include the last valid boundary."""
        if size <= window_size:
            return [0]
    ...
    def _sliding_window_inference(self, images):
        """Run tiled inference and merge patch predictions back to full image size."""
        bsz, _, h, w = images.shape
        window_size = self.slide_window_size
        stride = max(self.slide_stride, 1)
        ...
        y_starts = self._compute_starts(h_pad, window_size, stride)
        x_starts = self._compute_starts(w_pad, window_size, stride)

        pred_sum = None
        pred_count = torch.zeros((bsz, 1, h_pad, w_pad), device=self.device, dtype=torch.float32)

        for y0 in y_starts:
            for x0 in x_starts:
                image_patch = images[:, :, y0:y0 + window_size, x0:x0 + window_size]

                latents = self.vae.encode(image_patch).latent_dist.sample(generator=self.g) * 0.18215
                latents = latents.to(self.device)
                feats, _ = self.ldm_extractor.forward(latents)
                logits_patch, _ = self.network(feats, output_shape=(window_size, window_size))

                if pred_sum is None:
                    pred_sum = torch.zeros(
                        (bsz, logits_patch.shape[1], h_pad, w_pad),
                        device=self.device,
                        dtype=logits_patch.dtype,
                    )

                pred_sum[:, :, y0:y0 + window_size, x0:x0 + window_size] += logits_patch
                pred_count[:, :, y0:y0 + window_size, x0:x0 + window_size] += 1.0

        pred_full = pred_sum / pred_count.clamp_min(1.0).to(pred_sum.dtype)
    ...
    def validate(self, epoch, global_step):
        ...
        start_time = time.time()
        self.network.eval()
        if self.use_slide_window_inference:
            self.logger.info(
                f"Regression slide-window inference enabled: window={self.slide_window_size}, stride={self.slide_stride}"
            )
        ...
        with torch.no_grad():
            for idx, batch in tqdm(enumerate(self.val_loader), total=len(self.val_loader), leave=False):
                ...
                mse += F.mse_loss(logits.squeeze(dim=1), labels)
...
def val_log(cfg, network, ldm_extractor, val_loader, vae, g, epoch, global_step, logger, val_type="val", writer=None):
    ...
    Evaluators = {
        ...
        "change_detection": ChangeEvaluator,
